...
class Module01 extends KoanSuite with Matchers {
  ...
  ignore("should enforce limits on daily") {
    val tracker = new CalorieTracker
  ...
  ignore("should enforce limits on setting current daily calories based on tracker max") {
  ...
  ignore("should enforce limits on the eat and exercise methods too") {
    val tracker1 = new CalorieTracker
    val tracker2 = new CalorieTracker
  ...
  ignore("cache should not take a long time to retrieve recipes for the same food") {
